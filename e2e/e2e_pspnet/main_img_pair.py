	...
	for epoch in range(num_epochs):
		...
		with torch.no_grad():
			...
			for i in range(num_val_iters-1):
				...
				equalz = torch.sum(torch.all(torch.eq(y, y_pred), dim=1)).item()
				count = count + equalz
				total = total + val_batch_size

			small_val_acc = count/total
			is_best = (small_val_acc >= best_acc)
			print('Small Val Accuracy after Epoch {} : {}, is_best: {}'.format(epoch+1, small_val_acc, is_best)) 
			sys.stdout.flush()
		...
		if is_best:
			...
			torch.save(save_state, "./saved_models/model_arr_" + uniq_id + ".ckpt")
